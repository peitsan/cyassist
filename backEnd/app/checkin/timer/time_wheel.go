...
type timeWheel struct {
	...
	// 2个64槽位, 代表不同的刻度,两个即可，最大时间也就24h
	t2Tot5 [2][levelSize]*Time

	// 时间只精确到1s
	// curTimePoint 为1就是1s 为2就是2s
...
func (t *timeWheel) init() {
	...
	t.curTimePoint = get1s()
}

func maxVal() uint64 {
	return (1 << (nearShift + 2*levelShift)) - 1
}
...
func (t *timeWheel) AfterFunc(expire time.Duration, callback func()) TimeNoder {
	...
	expire = expire/(time.Second) + time.Duration(jiffies)
...
func getExpire(expire time.Duration, jiffies uint64) time.Duration {
	return expire/(time.Second) + time.Duration(jiffies)
}
...
// get1s函数通过参数传递，为了方便测试
func (t *timeWheel) run(get1s func() time.Duration) {
	// 先判断是否需要更新
